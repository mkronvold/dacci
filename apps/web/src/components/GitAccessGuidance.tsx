...
export function GitAccessGuidance(props: GitAccessGuidanceProps) {
  return (
    <p className={props.className ?? "muted"}>
      ...
      mounted host <code>~/.ssh</code> configuration plus any forwarded <code>ssh-agent</code> socket for pull, push,
      and refresh. Make sure the same remote already works from this machine first.
    </p>
  );
}
